...
#         #return html.Div([solution.DataTable(id='table',columns=mycolumns,data=coords.to_dict("rows"))])  
#         #columns =  [{"name": i, "id": i,} for i in (coords.columns)]
#         tbl = dash_table.DataTable(data = coords.to_dict('rows'),columns = [{'names' : i,'id' : i} for i in coords.columns])
#         return tbl
#     else:
...
#         # solution['color'] = color
#         # columns =  [{"name": i, "id": i,} for i in (solution.columns)]
#         # return dt.DataTable(data=solution, columns=columns) 
#         tbl = dash_table.DataTable(data = solution.to_dict('rows'),columns = [{'names' : i,'id' : i} for i in solution.columns])
#         return tbl
    
#         #mycolumns = [{'name': i, 'id': i} for i in solution.columns]
